...
def analyze_letter_frequencies(
    ...
    frequencies_per_position: list[dict[str, int]] = [
        {
            letter: sum(1 for word in word_set if word[index] == letter)
            for letter in allowed_letters
        }
        for index in range(max(len(word) for word in word_set))
    ]
...
class WordleGuesser(WordlePlayer):
    ...
    def prompt_word(self) -> str:
        ...
        look_for_letters: set[str] = {
            letter
            for word in self.available
            for letter in word
            if letter in self.unknown_letters
        }
        frequencies, frequencies_per_position = analyze_letter_frequencies(
            self.available, look_for_letters
        )

        # confidence grows over the span of a game to force it to make guesses instead of getting greedy with information
        endgame_confidence = (
            len(game.guess_history) / game.max_guesses
        ) ** CONFIDENCE_ENDGAME_GROWTH_RATE
        ...
        if total_confidence >= CONFIDENCE_THRESHOLD:
            ...
            for word in self.valid_dictionary:
                ...
                for index, letter in enumerate(word):
                    ...
                    frequency = frequencies.get(letter, 0)
                    positional_frequency = frequencies_per_position[index].get(
                        letter, 0
                    )
                    ...
                    score += (
                        ...
                        + positional_frequency * IGP_UNKNOWN_LETTER_MULT
                    ) * duplicate_penalty + known_penalty
    ...
    def on_guess_feedback(self, guessed_word: str, word_validity: list[LetterValidity]):
        ...
        for index, (letter, validity) in enumerate(zip(guessed_word, word_validity)):
            ...
            if (
                validity == LetterValidity.Incorrect
                and letter not in self.invalid_letters
            ):
            ...
            if validity == LetterValidity.TooMany:
                letter_limits[letter] = (
                    letter_limits.get(letter, guessed_word.count(letter)) - 1
                )
        ...
        for word in self.available:
            ...
            if any(
                True
                for index, letter in self.existing_letters.items()
            ...
            if any(
                (
                    True
                    for letter, count in letter_limits.items()
